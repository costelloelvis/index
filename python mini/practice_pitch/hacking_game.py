...
def main():
    ...
    print('''Hacking Minigame. Find the password in the computer's memory. You are given clues after
...
def getWords():
    """Return a list of 12 words that could possibly be the password.The secret password will be the first word in the list.To make the game fair, we try to ensure that there are words witha range of matching numbers of letters as the secret word."""
            ...
            for i in range(500):
                ...
                if numMatchingLetters(secretPassword, randomWord) == 3:
                    ...
                    for i in range(500):
                        ...
                        while len(words) < 12:randomWord = getOneWordExcept(words)
                        words.append(randomWord)
                        assert len(words) == 12
                        return words
...
def getComputerMemoryString(words):
    ...
    memoryAddress = 16 * random.randint(0, 4000)# Create the "computer memory" string.
    ...
    for lineNum in range(16): # The "computer memory" has 16 lines.
        ...
        for j in range(16): # Each half line has 16 characters.
            ...
            rightHalf += random.choice(GARBAGE_CHARS) # Fill in the password from words:
